...
export const PATCH = async (request: Request) => {
    try {
        ...
        if(!updatedUser) {return new NextResponse("User not found in the database", {status: 500});}
        

        
        return new NextResponse(JSON.stringify({message: "User successfully updated", user: updatedUser}), {status: 200});
     } catch (error: any) {
         return new NextResponse("Error in updatintg user" + error.message, {status: 500});
     }
};
